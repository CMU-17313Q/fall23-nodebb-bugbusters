...
async function logGroupEvent(caller, event, additional) {
  await events_1.default.log(Object.assign({
    type: event,
    uid: caller.uid,
    ip: caller.ip
  }, additional));
...
async function _delete(caller, data) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
  await isOwner(caller, groupName);
  if (groups_1.default.systemGroups.includes(groupName) || groups_1.default.ephemeralGroups.includes(groupName)) {
...
async function join(caller, data) {
  ...
  if (!isCallerAdmin && (groups_1.default.systemGroups.includes(groupName) || groups_1.default.isPrivilegeGroup(groupName))) {
    throw new Error('[[error:not-allowed]]');
  }
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
...
async function leave(caller, data) {
  ...
  const {
    displayname
  } = await user_1.default.getUserFields(data.uid, ['username']);
  // The next line calls a function in a module that has not been updated to TS yet
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
